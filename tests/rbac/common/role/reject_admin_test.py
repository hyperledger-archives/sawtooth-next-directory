...
@pytest.mark.role
class RejectRoleAddAdminTest(RoleTestHelper):
    ...
    def test_interface(self):
        ...
        self.assertIsInstance(self.role.admin.reject, RejectAddRoleAdmin)
        self.assertTrue(callable(self.role.admin.reject.address))
        self.assertTrue(callable(self.role.admin.reject.make))
        self.assertTrue(callable(self.role.admin.reject.make_addresses))
        self.assertTrue(callable(self.role.admin.reject.make_payload))
        self.assertTrue(callable(self.role.admin.reject.create))
        self.assertTrue(callable(self.role.admin.reject.send))
        self.assertTrue(callable(self.role.admin.reject.get))
    ...
    def test_make(self):
        ...
        message = self.role.admin.reject.make(
        ...
        self.assertIsInstance(message, protobuf.role_transaction_pb2.RejectAddRoleAdmin)
    ...
    def test_make_addresses(self):
        ...
        inputs, outputs = self.role.admin.reject.make_addresses(
    ...
    def test_make_payload(self):
        ...
        self.assertTrue(callable(self.role.admin.reject.make_payload))
        message, owner_key = self.test_make()
        payload = self.role.admin.reject.make_payload(
            message=message, signer_keypair=owner_key
        )

        proposal_address = self.role.admin.reject.address(
            object_id=message.role_id, target_id=message.user_id
        )
    ...
    def test_create(self):
        ...
        proposal, owner_key = self.get_testdata_role_admin_proposal()
        reason = self.get_testdata_reason()
        message = self.role.admin.reject.make(
        ...
        got, status = self.role.admin.reject.create(

...
def confirm_add_role_admins(
    ...
    confirm_add_payload = role_transaction_pb2.ConfirmAddRoleAdmin(
        ...
        role_id=role_id,
        user_id=user_id,
        reason=additional_params["reason"],
    )
...
def approve_add_role_members(
    ...
    confirm_payload = role_transaction_pb2.ApproveAddRoleMember(
        proposal_id=proposal_id,
        role_id=role_id,
        user_id=user_id,
        reason=additional_params["reason"],
        on_behalf_id=additional_params["on_behalf_id"],
    )

    inputs = [
        addresser.make_proposal_address(role_id, user_id),
        addresser.make_role_owners_address(role_id, additional_params["on_behalf_id"]),
    ]

    for user in additional_params["hierarchy_users"]:
        inputs.append(addresser.make_user_address(user))

...
def import_preexisting_ldap_inbound_records():
    ...
    LOGGER.info("db_user_payload: %s", most_recent_user_sync)

    if not most_recent_user_sync:
...
def _insert_users_into_inbound_queue(ldap_users):
    ...
    for ldap_user in ldap_users:
        ...
        ldap_record = ldap_payload_transformer.to_next_user_create(ldap_user)
...
def _insert_sync_event(data_type):
    ...
    yield from r.table(DB_TABLE_INBOUND_LDAP_SYNC_EVENT).insert(
        {
            DB_ATTR_LDAP_SYNC_EVENT_ID: current_time,
...
def _get_most_recent_sync_event(data_type):
    ...
    LOGGER.info("Getting latest sync time...")

    last_sync = yield from _get_latest_sync_or_none(connection, data_type)

    return last_sync


async def _get_latest_sync_or_none(connection, data_type):
    ...
    try:
        return (
            ...
            .max(DB_ATTR_LDAP_SYNC_EVENT_ID)
            .run(connection)
        )
...
async def _get_ldap_connection(server, username, password):
    # TODO: Add a timeout
    LOGGER.info("Getting ldap connection")

    server = Server(server, get_info=ALL)
    ldap_connection = Connection(server, user=username, password=password)
...
@asyncio.coroutine
def _wait_until_tables_are_available():
    """Create and return a connection to RethinkDb"""

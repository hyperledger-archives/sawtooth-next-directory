...
def _transfer_outbound_record_to_active_dir(outbound_entry):
    ...
    try:
        ...
        if outbound_entry[DB_ATTR_DATA_TYPE] == DB_VALUE_DATA_TYPE_USER:
            yield from _create_or_update_user_in_active_dir(payload_data_value)

        elif outbound_entry[DB_ATTR_DATA_TYPE] == DB_VALUE_DATA_TYPE_GROUP:
            yield from _create_or_update_group_in_active_dir(payload_data_value)
...
# TODO: Run this to tease out blocking issues (missing yields)
@asyncio.coroutine
def _create_or_update_group_in_active_dir(payload):
    """
    ...
    ldap_connection = yield from _get_ldap_connection(
        server=LDAP_SERVER, username=LDAP_USER, password=LDAP_PASS
    )
...
@asyncio.coroutine
def _create_or_update_user_in_active_dir(next_payload):
    """
...
@asyncio.coroutine
# TODO: This is duplicated from inbound svc. Move and consolidate into a universal function

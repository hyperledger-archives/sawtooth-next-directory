...
def validate_next_payload(payload):
    ...
    for required_data in [
        ...
        required_field_data_type,
        required_field_provider_id,
        required_value_data_type,
        required_field_dn,
        required_field_user_name,
        required_field_cn,
        required_field_given_name,
        required_field_name,
        required_field_manager,
    ]:

        data_node = payload[required_field_data]
        if required_data not in data_node:
            raise LdapValidationException(
                "'{0}' is missing an entry for: '{1}'".format(
                    required_field_data, required_data
                )
            )

        if not data_node[required_data]:
            raise LdapValidationException(
                "'{0}'.'{1}' cannot be empty".format(required_field_data, required_data)
            )

...
@authorized()
async def feed(request, w_s):
    """Socket feed enabling real-time notifications"""
    while True:
        required_fields = ["user_id"]
        recv = json.loads(await w_s.recv())

        utils.validate_fields(required_fields, recv)
        await proposal_feed(request, w_s, recv)


async def proposal_feed(request, w_s, recv):
    ...
    while await subscription.fetch_next():
        ...
        if (
            ...
            await w_s.send(json.dumps({"open_proposal": proposal_resource}))
        elif recv.get("user_id") == proposal_resource["opener"]:
            await w_s.send(json.dumps({"user_proposal": proposal_resource}))

...
class ConfirmAddRoleAdmin(ProposalConfirm):
    """Implements the CONFIRM_ADD_ROLE_ADMIN message
    usage: rbac.role.admin.confirm.create()"""
    ...
    def make_addresses(self, message, signer_user_id):
        ...
        admin_address = addresser.user.address(message.related_id)
        inputs.add(admin_address)
        outputs.add(admin_address)
    ...
    def validate_state(self, context, message, payload, input_state, store):
        """Validates that:
        1. the signer is an admin of the role"""

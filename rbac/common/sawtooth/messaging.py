...
# ------------------------------------------------------------------------------
#
# Borrowed from python Sawtooth Rest API:
# https://github.com/hyperledger/sawtooth-core/blob/master/rest_api/sawtooth_rest_api/messaging.py
#
# ------------------------------------------------------------------------------
...
class _Backoff:
    ...
    async def do_backoff(self, err_msg=" "):
        if self.num_retries == self.max_retries:
            LOGGER.warning(
                "Failed sending message to the Validator. No more "
        ...
        self.num_retries += 1
        LOGGER.warning(
            "Sleeping for %s ms after failed attempt %s of %s to "
    ...
    def _set_reply(self, correlation_id, msg):
        if correlation_id in self._futures:
            try:
                ...
                LOGGER.error(
                    "Attempting to set result on already-resolved future: %s", str(e)
                )

    def _fail_reply(self, correlation_id, err):
        if correlation_id in self._futures and not self._futures[correlation_id].done():
            try:
                self._futures[correlation_id].set_exception(err)
            except asyncio.InvalidStateError as e:
                LOGGER.error(
                    "Attempting to set exception on already-resolved future: " "%s",
                    str(e),
                )
...
class _Sender:
    ...
    async def send(self, message_type, message_content, timeout=None):
        ...
        backoff = _Backoff(max_retries=3, interval=200, error=SendBackoffTimeoutError())
...
class Connection:
    ...
    async def _do_start(self, reconnect=False):
        ...
        self._monitor_fd = "inproc://monitor.s-{}".format(uuid.uuid4().hex[0:5])
    ...
    async def send(self, message_type, message_content, timeout=None):
        ...
        """
        return await self._sender.send(message_type, message_content, timeout=timeout)
